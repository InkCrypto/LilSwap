...
export const CollateralSwapModal = ({
    ...
    const { marketAssets: fetchedMarketAssets, supplies, summary, loading: positionsLoading, refresh: refreshPositions } = useUserPosition();
    ...
    const modalTitle = useMemo(() => {
        if (fromToken && toToken) return `Collateral Swap: ${fromToken.symbol} → ${toToken.symbol}`;
        if (fromToken) return `Collateral Swap: ${fromToken.symbol}`;
                                        ...
                                        {(() => {
                                            ...
                                            if (feeBps > 0) {
                                                try {
                                                    ...
                                                    // In CollateralSwap, toToken corresponds to destAmount
                            ...
                            <div className="px-1 pb-2 pt-3 space-y-3 border-t border-slate-200 dark:border-slate-700/60">
                                ...
                                <div className="flex justify-between items-center text-[13px] text-slate-600 dark:text-slate-300 font-medium">
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <InfoTooltip content="Annual percentage yield you will earn on your deposited assets." size={12} />
                                    </div>
                                ...
                                <div className="flex justify-between items-center text-[13px] text-slate-600 dark:text-slate-300 font-medium">
                                    ...
                                    <div className="text-right flex items-center gap-1.5">
                                        {(() => {
                                            const fromAddr = (fromToken?.underlyingAsset || fromToken?.address || '').toLowerCase();
                                            const fromMarketToken = (marketAssets || []).find(m => (m.underlyingAsset || m.address || '').toLowerCase() === fromAddr);
                                            const currentLT = (parseFloat(fromMarketToken?.reserveLiquidationThreshold) || 0) * 100;

                                            const toAddr = (toToken?.underlyingAsset || toToken?.address || '').toLowerCase();
                                            const toMarketToken = (marketAssets || []).find(m => (m.underlyingAsset || m.address || '').toLowerCase() === toAddr);
                                            const newLT = (parseFloat(toMarketToken?.reserveLiquidationThreshold) || 0) * 100;
                                ...
                                <div className="flex justify-between items-center text-[13px] text-slate-600 dark:text-slate-300 font-medium pb-1">
                                    ...
                                    <div className="text-right flex items-center gap-1.5">
                                        {(() => {
                                            const activeSupplies = supplies || fallbackSupplies || providedSupplies || [];
                                            ...
                                            try {
                                                ...
                                                const existingFromBalance = existingFromSupply ? parseFloat(existingFromSupply.formattedAmount) : 0;
